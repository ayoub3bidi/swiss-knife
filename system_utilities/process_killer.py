    ...
    def execute(self) -> None:
        ...
        for proc in targets:
            ...
            if info:
                ...
                print(
                    f"{info['pid']:<8} {info['name'][:24]:<25} {info['username'][:14]:<15} "
        ...
        for proc, info in iterator:
            if self.kill_process(proc):
                ...
                print(
                    f"{status} {info['name']} (PID {info['pid']}) - "
...
WARNING: Use with extreme caution! Killing processes can cause data loss.
    ...
    parser.add_argument(
        "--cpu-threshold", type=float, help="Kill if CPU percent exceeds this (0-100+)"
    )
    parser.add_argument(
        "--min-memory", type=float, help="Kill if memory usage exceeds this (MB)"
    )
    ...
    parser.add_argument(
        "--top", type=int, metavar="N", help="Kill top N processes by memory usage"
    )
    ...
    if not any([args.memory_threshold, args.cpu_threshold, args.min_memory]):
        parser.error(
            "Must specify at least one threshold: --memory-threshold, --cpu-threshold, or --min-memory"
        )
