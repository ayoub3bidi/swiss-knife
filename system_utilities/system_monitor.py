...
class ResourceMonitor:
    ...
    def _get_network_info(self) -> Dict:
        ...
        if time_delta > 0:
            bytes_sent_per_sec = (current_net.bytes_sent - self.last_network.bytes_sent) / time_delta
            bytes_recv_per_sec = (current_net.bytes_recv - self.last_network.bytes_recv) / time_delta
        else:
    ...
    def _get_process_info(self, limit: int = 5) -> List[Dict]:
        ...
        # Sort by CPU usage
        top_cpu = sorted(processes, key=lambda p: p['cpu_percent'] or 0, reverse=True)[:limit]
        
        # Sort by memory usage
        top_mem = sorted(processes, key=lambda p: p['memory_percent'] or 0, reverse=True)[:limit]
    ...
    def check_resources(self) -> Dict:
        ...
        if disk_info['partitions']:
            avg_disk = sum(p['percent'] for p in disk_info['partitions'].values()) / len(disk_info['partitions'])
    ...
    def check_thresholds(self, data: Dict) -> List[Dict]:
        ...
        if data['cpu']['percent'] > self.thresholds['cpu']:
            if self.alert_manager.should_alert('cpu'):
                alert = self.alert_manager.record_alert(
                    ...
                    {'usage': data['cpu']['percent'], 'threshold': self.thresholds['cpu']}
                )
                alerts.append(alert)
        ...
        # Network alert
        total_network_speed = data['network']['speed_sent_mbps'] + data['network']['speed_recv_mbps']
    ...
    def print_status(self, data: Dict, alerts: List[Dict]):
        ...
        print(f"   Cores: {cpu['count']} | Load Average: {cpu['load_average']['1min']:.2f}, "
        ...
        print(f"   Used: {self._format_bytes(mem['used'])} / {self._format_bytes(mem['total'])}")
        print(f"   Available: {self._format_bytes(mem['available'])}")
        
        if mem['swap_total'] > 0:
            print(f"   Swap: {mem['swap_percent']:.1f}% ({self._format_bytes(mem['swap_used'])} / "
                  f"{self._format_bytes(mem['swap_total'])})")
        
        # Disk
        print("\n💾 Disk Usage:")
        for mountpoint, disk_data in sorted(data['disk']['partitions'].items()):
            disk_status = "🔴" if disk_data['percent'] > self.thresholds['disk'] else "🟢"
            print(f"   {disk_status} {mountpoint}: {disk_data['percent']:.1f}% - "
        ...
        print(f"   Connections: {net['connections']['established']} established, "
              f"{net['connections']['listen']} listening")
        print(f"   Total: ↑ {self._format_bytes(net['bytes_sent'])} | ↓ {self._format_bytes(net['bytes_recv'])}")
        ...
        # Statistics
        print("\n📊 Statistics:")
        print(f"   Checks: {self.stats['checks_performed']} | Alerts: {self.stats['alerts_triggered']}")
        ...
        if len(self.history['cpu']) > 1:
            ...
            print(f"   Trends: CPU {cpu_arrow} {abs(cpu_trend):.1f}% | Memory {mem_arrow} {abs(mem_trend):.1f}%")
            ...
            while self.running:
                ...
                if show_processes:
                    ...
                    for proc in proc_info['top_cpu']:
                        print(f"   {proc['name'][:30]:30} | CPU: {proc['cpu_percent']:5.1f}% | "
...
Examples:
  ...
  python system_monitor.py --once
    ...
    parser.add_argument('--log', type=str,
                       help='Log file path for continuous monitoring data')
