...
Analyze directory sizes, find space hogs, and generate visual reports.
"""
...
class DiskAnalyzer:
    ...
    def _get_directory_size(self, path: Path, current_depth: int = 0) -> FileStats:
        """Recursively calculate directory size and statistics."""
        total_size = 0
        file_count = 0
        dir_count = 0
        largest_file = None
    ...
    def get_subdirectory_sizes(self, path: Path, top_n: int = 20) -> List[Tuple[str, int]]:
        """Get sizes of immediate subdirectories, sorted by size."""
        subdir_sizes = []
    ...
    def print_tree_view(self, stats: FileStats, indent: int = 0, 
                       max_items: int = 10, size_threshold: float = 0.01):
        """Print hierarchical tree view of directories."""
        prefix = "  " * indent
        size_str = self._format_size(stats.size)
        percent = (stats.size / self.total_size * 100) if self.total_size > 0 else 0
        
        print(f"{prefix}├── {Path(stats.path).name or stats.path} ({size_str}, {percent:.1f}%)")
    ...
    def print_bar_chart(self, data: List[Tuple[str, int]], title: str, 
        ...
        for name, size in top_items:
            ...
            print(f"{display_name:<42} {bar:<{max_width}} {size_str:>12} ({percent:>5.1f}%)")
    
    def print_summary(self, stats: FileStats):
        """Print analysis summary with visualizations."""
        print("\n" + "=" * 80)
        ...
        if self.extension_stats:
            ext_list = [(ext, data['size'], data['count']) 
        ...
        if self.largest_files:
            ...
            largest = [(Path(path).name, size) for size, path in self.largest_files[:20]]
        ...
        data = {
            ...
            'extension_stats': {
                ext: {
                    ...
                    'percentage': (data['size'] / self.total_size * 100) if self.total_size > 0 else 0
                }
                for ext, data in sorted(self.extension_stats.items(), 
            ...
            'subdirectories': [
                ...
                for name, size in self.get_subdirectory_sizes(Path(stats.path), top_n=100)
            ],
            'errors': [
                {'path': path, 'error': error}
                for path, error in self.errors[:100]
            ]
        }
