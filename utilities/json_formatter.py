...
class JSONFormatter:
    ...
    def format_directory(
        ...
        iterator = (
            tqdm(json_files, desc="Formatting", unit="files")
            if HAS_TQDM
            else json_files
        )
    ...
    def convert_format(
        ...
        if output_format == "json":
            ...
            if isinstance(data, list):
                lines = [
                    json.dumps(item, ensure_ascii=self.ensure_ascii) for item in data
                ]
                content = "\n".join(lines)
            else:
    ...
    def print_stats(self):
        ...
        print(
            f"Output size:       {self._format_size(self.stats['total_output_size'])}"
        )
...
Examples:
  ...
  python json_formatter.py file1.json file2.json --merge -o merged.json
    ...
    parser.add_argument(
        "--sort-keys", action="store_true", help="Sort object keys alphabetically"
    )
    parser.add_argument(
        "--ascii",
        dest="ensure_ascii",
    ...
    parser.add_argument(
        "--merge", action="store_true", help="Merge multiple JSON files"
    )
    parser.add_argument(
        "--merge-as-object",
        action="store_true",
    ...
    parser.add_argument(
        "--minify", action="store_true", help="Shorthand for --mode minify"
    )
    ...
    try:
        formatter = JSONFormatter(
            sort_keys=args.sort_keys, ensure_ascii=args.ensure_ascii
        )
        ...
        if len(args.input) == 1 and args.input[0].is_file():
            formatter.format_file(
                args.input[0], args.output, args.mode, args.indent, args.backup
            )
        ...
        # Multiple files
            ...
            for filepath in args.input:
                if filepath.is_file():
                    ...
                    formatter.format_file(
                        filepath, output_path, args.mode, args.indent, args.backup
                    )
