...
class ExcelReportGenerator:
    ...
    def add_table(
        ...
        if auto_filter:
            self.worksheet.auto_filter.ref = (
                f"A{start_row}:{get_column_letter(len(headers))}{self.current_row - 1}"
            )
        ...
        for col_idx, header in enumerate(headers, start=1):
            ...
            self.worksheet.column_dimensions[column_letter].width = min(
                max_length + 2, 50
            )
    ...
    def add_summary(self, data: List[Dict], numeric_columns: List[str]):
        ...
        self.worksheet.cell(row=self.current_row, column=start_col).font = Font(
            bold=True
        )
    ...
    def add_chart(
        self, chart_type: str, data_range: str, title: str, position: str = None
    ):
        if chart_type == "bar":
            chart = BarChart()
