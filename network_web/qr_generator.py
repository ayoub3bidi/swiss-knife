...
class QRGenerator:
    ...
    def __init__(
        ...
        self.error_correction = error_map.get(
    ...
    def _add_label(
        self, img: Image.Image, label: str, padding: int = 20, font_size: int = 24
    ) -> Image.Image:
        if not HAS_PIL:
            return img
    ...
    def generate_batch_from_file(
        self, filepath: Path, output_dir: Path, name_template: str = "qr_{:03d}.png"
    ):
        try:
            ...
            iterator = (
                tqdm(
                    enumerate(lines, 1), total=len(lines), desc="Generating", unit="QR"
                )
                if HAS_TQDM
                else enumerate(lines, 1)
            )
    ...
    def generate_batch_from_json(self, filepath: Path, output_dir: Path):
        try:
            ...
            for item in iterator:
                ...
                output_name = item.get(
                    "output", f"qr_{qr_type}_{self.stats['generated'] + 1}.png"
                )
...
""",
    ...
    mode.add_argument(
        "--vcard", action="store_true", help="Generate vCard (contact) QR code"
    )
    ...
    parser.add_argument(
        "--vcard-phone", dest="vcard_phone", type=str, help="Contact phone"
    )
    parser.add_argument(
        "--vcard-email", dest="vcard_email", type=str, help="Contact email"
    )
    ...
    parser.add_argument(
        "--output-dir", type=Path, help="Output directory for batch generation"
    )
    ...
    parser.add_argument(
        "--box-size", type=int, default=10, help="Box size in pixels (default: 10)"
    )
    parser.add_argument(
        "--border", type=int, default=4, help="Border size in boxes (default: 4)"
    )
    parser.add_argument(
        "--fill-color", default="black", help="Foreground color (name or #RRGGBB)"
    )
    parser.add_argument(
        "--back-color", default="white", help="Background color (name or #RRGGBB)"
    )
    parser.add_argument(
        "--style",
        default="square",
    ...
    try:
        ...
        if args.url:
            ...
            generator.generate_email(
                args.email, args.subject, args.body, args.output, args.label
            )
