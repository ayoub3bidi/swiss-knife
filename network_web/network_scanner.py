...
class NetworkScanner:
    ...
    def __init__(
        self, timeout: float = 1.0, max_workers: int = 100, ping_timeout: float = 1.0
    ):
        """
        Args:
    ...
    def scan_network(
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            if HAS_TQDM:
                iterator = tqdm(
                    iterator, total=len(futures), desc="Scanning", unit="host"
                )
    ...
    def quick_scan(self, network: str) -> List[Dict]:
        ...
        return self.scan_network(
            network, common_ports, check_mac=False, grab_banners=False
        )

    def full_scan(self, network: str) -> List[Dict]:
        ports = list(self.COMMON_PORTS.keys())
        return self.scan_network(
            network, ports, check_hostname=True, check_mac=True, grab_banners=True
        )

    def scan_single_host(
        self, ip: str, port_range: Tuple[int, int] = (1, 1024)
    ) -> Optional[Dict]:
        ...
        result = self.scan_host(
            ip, ports, check_hostname=True, check_mac=True, grab_banners=True
        )
        self.stats["scan_end"] = datetime.now().isoformat()
...
""",
    ...
    mode.add_argument(
        "--quick", action="store_true", help="Quick scan (common ports only)"
    )
    mode.add_argument(
        "--full", action="store_true", help="Full scan (all common ports with banners)"
    )
    mode.add_argument(
        "--ports", type=str, help="Specific ports (comma-separated or range)"
    )
    ...
    parser.add_argument(
        "--banners", action="store_true", help="Attempt to grab service banners"
    )
    parser.add_argument(
        "--only-alive", action="store_true", help="Only show hosts with open ports"
    )

    # Output options
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Verbose output with banners"
    )
