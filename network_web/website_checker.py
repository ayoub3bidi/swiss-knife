...
class WebsiteChecker:
    ...
    def print_results(self, verbose: bool = False):
        ...
        for result in self.results:
            ...
            if result.get("ssl_info") and not result["ssl_info"].get("error"):
                ...
                if verbose and ssl.get("issuer"):
                    print(
                        f"  SSL Issuer: {ssl['issuer'].get('organizationName', 'Unknown')}"
                    )
        ...
        success_rate = (
            (self.stats["successful"] / self.stats["total_checked"] * 100)
...
""",
    ...
    parser.add_argument(
        "--file", "-f", type=Path, help="File containing URLs (one per line)"
    )
