...
class CodeFormatter:
    ...
    FORMATTERS = {
        ...
        'java': {
            ...
            'check_only_cmd': ['google-java-format', '--dry-run', '--set-exit-if-changed', '{file}'],
    ...
    def _format_file(self, filepath: Path, language: str) -> Dict:
        ...
        if not self.available_formatters.get(language, False):
            ...
            result['error'] = f"{config['tool']} not installed. Install: {config['install']}"
            self.stats['files_skipped'] += 1
            return result
        
        # Choose command based on check_only flag
        if self.check_only:
            cmd = [c.format(file=str(filepath)) if '{file}' in c else c 
    ...
    def format_files(self, filepaths: List[Path]) -> List[Dict]:
        ...
        iterator = tqdm(files_to_format, desc=mode, unit="file") if HAS_TQDM else files_to_format
    ...
    def format_directory(self, directory: Path, recursive: bool = True,
        ...
        exclude.extend(['node_modules', '.git', '__pycache__', 'venv', '.venv', 'dist', 'build'])
    ...
    def print_summary(self):
        ...
        if self.check_only:
            print(f"Files OK:           {self.stats['files_checked'] - self.stats['needs_formatting']}")
        ...
        missing = [lang for lang, available in self.available_formatters.items() if not available]
        if missing:
