...
class PythonDeadCodeDetector:
        ...
        for node in ast.walk(tree):
            ...
            if isinstance(node, ast.FunctionDef):
                if self._should_include_name(node.name, "function"):
                    self.definitions[node.name].append(
                        (filepath, node.lineno, "function")
                    )
                ...
                for target in node.targets:
                    if isinstance(target, ast.Name):
                        if self._should_include_name(target.id, "variable"):
                            self.definitions[target.id].append(
                                (filepath, node.lineno, "variable")
                            )
    ...
    def _calculate_confidence(self, name: str, item_type: str, usage_count: int) -> str:
    ...
    def identify_dead_code(self) -> List[DeadCodeItem]:
        ...
        for name, occurrences in self.definitions.items():
            ...
            for filepath, line_no, item_type in occurrences:
                ...
                if (
                    ...
                    < confidence_levels[self.min_confidence]
                ):
                    continue
                ...
                reason = (
                    "No references found"
                    if usage_count == 0
                    else f"Only {usage_count} reference(s)"
                )
        ...
        self.dead_code.sort(
            key=lambda x: (confidence_order[x.confidence], x.filepath, x.line_number)
        )
    ...
    def analyze_directory(self, directory: Path, recursive: bool = True) -> None:
        ...
        # First pass: collect definitions
        iterator = (
            tqdm(files, desc="Pass 1: Definitions", unit="files") if HAS_TQDM else files
        )
        ...
        # Second pass: collect usages
        iterator = (
            tqdm(files, desc="Pass 2: Usages", unit="files") if HAS_TQDM else files
        )
        for filepath in iterator:
    ...
    def print_results(self, show_all: bool = False, max_items: int = 50) -> None:
        ...
        for confidence in ["high", "medium", "low"]:
            ...
            for item in display_items:
                ...
                print(
                    f"  {item.type:10s} {item.name:30s} {filepath}:{item.line_number}"
                )
                print(f"             {item.reason}")
    ...
    def export_json(self, output_path: Path) -> None:
        data = {
            ...
            "by_type": {
                "functions": [
                    asdict(item) for item in self.dead_code if item.type == "function"
                ],
                "classes": [
                    asdict(item) for item in self.dead_code if item.type == "class"
                ],
                "variables": [
                    asdict(item) for item in self.dead_code if item.type == "variable"
                ],
                "imports": [
                    asdict(item) for item in self.dead_code if item.type == "import"
                ],
            },
            "by_confidence": {
                "high": [
                    asdict(item) for item in self.dead_code if item.confidence == "high"
                ],
                "medium": [
                    asdict(item)
                    for item in self.dead_code
                ...
                "low": [
                    asdict(item) for item in self.dead_code if item.confidence == "low"
                ],
            },
        }
...
Confidence Levels:
  ...
  LOW    - Has some references (likely used)
    ...
    try:
        ...
        detector.print_results(show_all=args.show_all, max_items=args.max_items)
