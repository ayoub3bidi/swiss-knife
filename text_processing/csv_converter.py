...
class CSVConverter:
    ...
    def _is_empty_row(self, row: Dict) -> bool:
        ...
        return all(not v or (isinstance(v, str) and not v.strip()) for v in row.values())
    ...
    def to_json(self, data: List[Dict], pretty: bool = True, 
    ...
    def to_xml(self, data: List[Dict], root_tag: str = 'root',
               row_tag: str = 'row', pretty: bool = True) -> str:
    ...
    def print_stats(self, input_path: Path, output_path: Path):
        ...
        print(f"Rows converted: {self.stats['rows_processed'] - self.stats['rows_skipped']}")
...
Examples:
  ...
  python csv_converter.py data.csv -f json --sort-keys
    ...
    parser.add_argument('--keep-empty-rows', dest='skip_empty_rows', action='store_false',
                       help='Keep rows with all empty fields')
    parser.add_argument('--skip-empty-fields', action='store_true',
                       help='Remove empty fields from output')
    parser.add_argument('--no-strip', dest='strip_whitespace', action='store_false',
                       help='Don\'t strip whitespace from values')
    parser.add_argument('--no-infer-types', dest='infer_types', action='store_false',
    ...
    # XML options
    parser.add_argument('--root-tag', default='root',
                       help='XML root element tag (default: root)')
    parser.add_argument('--row-tag', default='row',
                       help='XML row element tag (default: row)')
