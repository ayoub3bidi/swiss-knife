...
class WordFrequencyAnalyzer:
    ...
    DEFAULT_STOP_WORDS = {
        ...
        'very', 'can', 'just', 'should', 'now', 'i', 'you', 'we', 'or',
        'been', 'were', 'would', 'there', 'their', 'if', 'about', 'into',
        'through', 'during', 'before', 'after', 'above', 'below', 'up',
        'down', 'out', 'off', 'over', 'under', 'again', 'further', 'then',
        'once', 'here', 'any', 'my', 'our', 'your', 'his', 'her', 'me',
        'him', 'them', 'she', 'us', 'do', 'does', 'did', 'doing', 'am'
    }
    ...
    def _should_include_word(self, word: str) -> bool:
        """Check if word should be included based on filters."""
        # Case handling
    ...
    def _extract_context(self, text: str, word: str, context_words: int = 5) -> str:
        """Extract context around a word occurrence."""
        words = text.split()
    ...
    def analyze_file(self, filepath: Path, store_context: bool = False) -> Counter:
        """Analyze word frequencies in a file."""
        try:
    ...
    def analyze_directory(self, directory: Path, extensions: Optional[List[str]] = None,
                          recursive: bool = False, store_context: bool = False) -> None:
    ...
    def get_top_words(self, n: int = 20) -> List[Tuple[str, int]]:
        ...
        filtered = {word: count for word, count in self.word_frequencies.items()
    ...
    def get_words_starting_with(self, prefix: str, n: int = 20) -> List[Tuple[str, int]]:
        """Get most frequent words starting with prefix."""
        prefix_lower = prefix.lower()
        matching = {word: count for word, count in self.word_frequencies.items()
    ...
    def get_words_ending_with(self, suffix: str, n: int = 20) -> List[Tuple[str, int]]:
        """Get most frequent words ending with suffix."""
        suffix_lower = suffix.lower()
        matching = {word: count for word, count in self.word_frequencies.items()
                   if word.lower().endswith(suffix_lower)}
        return Counter(matching).most_common(n)
    
    def get_words_containing(self, substring: str, n: int = 20) -> List[Tuple[str, int]]:
        """Get most frequent words containing substring."""
    ...
    def get_word_statistics(self) -> Dict:
        ...
        return {
            ...
            'avg_word_length': self.stats['total_characters'] / self.stats['total_words'],
    ...
    def print_bar_chart(self, data: List[Tuple[str, int]], title: str, 
        ...
        for word, count in data:
            ...
            percentage = (count / self.stats['total_words'] * 100) if self.stats['total_words'] > 0 else 0
            
            print(f"{display_word:<22} {bar:<{max_width}} {count:>8} ({percentage:>5.2f}%)")
    ...
    def print_analysis(self, top_n: int = 20, show_contexts: bool = False) -> None:
        ...
        if len(self.file_frequencies) > 1:
            print(f"\nPER-FILE BREAKDOWN")
            print("=" * 80)
            for filepath, counter in sorted(self.file_frequencies.items(), 
...
Examples:
  ...
  python word_frequency.py doc.txt --only-alpha --min-frequency 5
    ...
    parser.add_argument('--case-sensitive', action='store_true',
                       help='Preserve word case')
    parser.add_argument('--remove-stop-words', action='store_true',
                       help='Remove common stop words')
    parser.add_argument('--stop-words-file', type=Path,
                       help='File with custom stop words (one per line)')
    parser.add_argument('--only-alpha', dest='only_alphabetic', action='store_true',
                       help='Only include alphabetic words')
    parser.add_argument('--no-numbers', dest='include_numbers', action='store_false',
    ...
    parser.add_argument('--contexts', action='store_true',
                       help='Show example contexts for top words')
    
    # Export options
    parser.add_argument('--export-json', type=Path,
                       help='Export results to JSON file')
    parser.add_argument('--export-csv', type=Path,
                       help='Export word frequencies to CSV file')
    
    # Search options
    parser.add_argument('--starts-with', type=str,
                       help='Show words starting with prefix')
    parser.add_argument('--ends-with', type=str,
                       help='Show words ending with suffix')
    parser.add_argument('--contains', type=str,
                       help='Show words containing substring')
    ...
    try:
        ...
        if args.starts_with:
            ...
            analyzer.print_bar_chart(results, f"WORDS STARTING WITH '{args.starts_with}'")
