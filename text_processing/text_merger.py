...
class TextMerger:
    ...
    def _collect_files(self, paths: List[str], extensions: Optional[List[str]] = None) -> List[Path]:
    ...
    def _process_lines(self, lines: List[str], start_line_num: int = 1) -> List[str]:
        """Process lines according to options."""
        processed = []
    ...
    def merge_files(self, filepaths: List[Path], output_path: Optional[Path] = None) -> str:
        ...
        # Progress bar
        iterator = tqdm(filepaths, desc="Merging files", unit="files") if HAS_TQDM else filepaths
    ...
    def merge_by_pattern(self, pattern: str, output_path: Path, 
    ...
    def merge_directories(self, directories: List[Path], output_path: Path,
                         extensions: Optional[List[str]] = None,
                         recursive: bool = False) -> None:
        """Merge all files from multiple directories."""
        patterns = []
...
Sort options: name, size, date, extension
    ...
    parser.add_argument('files', nargs='+',
                       help='Files, directories, or glob patterns to merge')
    parser.add_argument('-o', '--output', type=Path, required=True,
                       help='Output file path')
    
    # Delimiter options
    parser.add_argument('-d', '--delimiter', default='line',
                       help='Delimiter between files (preset name or custom string)')
    parser.add_argument('--no-filename', dest='add_filename', action='store_false',
    ...
    parser.add_argument('--stats', dest='add_stats', action='store_true',
                       help='Add file statistics (lines, size) to headers')
    
    # Sorting options
    parser.add_argument('--sort', choices=['name', 'size', 'date', 'extension'],
                       dest='sort_by',
    ...
    # Content processing
    parser.add_argument('--remove-empty', dest='remove_empty', action='store_true',
                       help='Remove empty lines from output')
    parser.add_argument('--strip', dest='strip_whitespace', action='store_true',
                       help='Strip leading/trailing whitespace from lines')
    parser.add_argument('--line-numbers', dest='add_line_numbers', action='store_true',
    ...
    parser.add_argument('--on-error', choices=['skip', 'stop', 'replace'], 
                       default='skip', dest='error_handling',
                       help='How to handle read errors (default: skip)')
    
    # Preview
    parser.add_argument('--preview', action='store_true',
                       help='Preview merge without writing file')
    parser.add_argument('--preview-lines', type=int, default=50,
