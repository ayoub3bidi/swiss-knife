...
class DatabaseBackup:
    ...
    def _compress_file(self, filepath: Path) -> Path:
        ...
        print(f"Compressed: {self._format_size(original_size)} → {self._format_size(compressed_size)} ({ratio:.1f}%)")
    ...
    def _backup_mysql(self, host: str, port: int, user: str, password: str,
                     database: str, output_file: Path) -> bool:
    ...
    def _backup_postgresql(self, host: str, port: int, user: str, password: str,
                          database: str, output_file: Path) -> bool:
    ...
    def _backup_mongodb(self, host: str, port: int, user: Optional[str],
                       password: Optional[str], database: str, output_file: Path) -> bool:
    ...
    def backup(self, host: str = 'localhost', port: Optional[int] = None,
              user: Optional[str] = None, password: Optional[str] = None,
              database: str = None, database_path: Optional[str] = None) -> Optional[Path]:
        ...
        if self.db_type == 'mysql':
            success = self._backup_mysql(host, port, user, password, database, output_file)
        elif self.db_type == 'postgresql':
            success = self._backup_postgresql(host, port, user, password, database, output_file)
        elif self.db_type == 'mongodb':
            success = self._backup_mongodb(host, port, user, password, database, output_file)
        ...
        # Compress if requested and not MongoDB (already compressed)
    ...
    def rotate_backups(self) -> None:
        ...
        # Collect all backups with dates
        ...
        for filepath, backup_date in backups:
            ...
            if age_days < self.keep_days + (self.keep_weekly * 7) + (self.keep_monthly * 30):
            ...
            if filepath.suffix in ['.sql', '.dump', '.archive', '.gz'] or \
               (filepath.suffix == '.db' and self.db_type == 'sqlite'):
                ...
                backup_info = {
                    ...
                    'age_days': (datetime.now() - backup_date).days if backup_date else None,
                    'size': size,
    ...
    def print_backups(self) -> None:
        ...
        for backup in backups:
            age = f"{backup['age_days']}d ago" if backup['age_days'] is not None else "unknown"
            print(f"{backup['filename']:50s} {backup['size_formatted']:>10s} {age:>12s}")
...
Examples:
  ...
  python database_backup.py mysql -o backups/ --rotate-only
    ...
    parser.add_argument('db_type', choices=['mysql', 'postgresql', 'mongodb', 'sqlite'],
                       help='Database type')
