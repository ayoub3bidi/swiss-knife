...
class ScreenshotScheduler:
    ...
    def _print_status(self, filepath: Path, elapsed: float):
        """Print status update."""
        avg_size = self.total_size / self.screenshot_count if self.screenshot_count > 0 else 0
    ...
    def run(self) -> None:
        """Run the screenshot scheduler."""
        print("=" * 80)
        print("SCREENSHOT SCHEDULER")
        print("=" * 80)
        print(f"Output directory: {self.output_dir}")
        print(f"Interval:         {self._format_duration(self.interval)}")
        print(f"Duration:         {'Unlimited' if not self.duration else self._format_duration(self.duration)}")
        print(f"Max screenshots:  {self.max_screenshots or 'Unlimited'}")
        print(f"Monitor:          {'All' if self.monitor == -1 else f'#{self.monitor}'}")
    ...
    def _print_summary(self):
        ...
        print(f"Average size:       {self._format_size(self.total_size / self.screenshot_count) if self.screenshot_count > 0 else '0 B'}")
        ...
        metadata = {
            ...
            'statistics': {
                ...
                'average_size_bytes': self.total_size // self.screenshot_count if self.screenshot_count > 0 else 0
            },
            'settings': {
                'format': self.format,
...
def parse_duration(duration_str: str) -> int:
    """Parse duration string (e.g., '1h', '30m', '1h30m') to seconds."""
    total_seconds = 0
...
Duration format: 1h30m, 30m, 90s, or just seconds
    ...
    parser.add_argument('-d', '--duration', type=str,
                       help='Total duration to run (e.g., 2h, 30m)')
    parser.add_argument('--max', '--max-screenshots', dest='max_screenshots', type=int,
                       help='Maximum number of screenshots to take')
    
    parser.add_argument('--filename', type=str, default='screenshot_{timestamp}.png',
    ...
    parser.add_argument('--resize', type=str,
                       help='Resize screenshots (e.g., 1920x1080)')
