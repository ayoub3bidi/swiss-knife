...
class PasswordGenerator:
    ...
    def _has_sequential_chars(self, password: str) -> bool:
        ...
        for i in range(len(password_lower) - 2):
            if ord(password_lower[i]) + 1 == ord(password_lower[i + 1]) and ord(
                password_lower[i + 1]
            ) + 1 == ord(password_lower[i + 2]):
                return True
    ...
    def _generate_attempt(self) -> str:
        ...
        if self.policy.min_uppercase > 0:
            chars.extend(
                secrets.choice(uppercase) for _ in range(self.policy.min_uppercase)
            )

        if self.policy.min_lowercase > 0:
            chars.extend(
                secrets.choice(lowercase) for _ in range(self.policy.min_lowercase)
            )
        ...
        if self.policy.min_special > 0:
            chars.extend(
                secrets.choice(special) for _ in range(self.policy.min_special)
            )
...
Examples:
  ...
  python password_generator.py --special "!@#$%"
    ...
    parser.add_argument(
        "--min-digits", type=int, default=1, help="Minimum digits (default: 1)"
    )
    parser.add_argument(
        "--min-special",
        type=int,
    ...
    parser.add_argument(
        "--no-similar",
        dest="exclude_similar",
        action="store_true",
        help="Exclude similar characters (il1|!I/\\0O)",
    )
    parser.add_argument("--special", type=str, help="Custom special characters")
    ...
    parser.add_argument(
        "--separator", default="-", help="Passphrase word separator (default: -)"
    )
    parser.add_argument(
        "--no-capitalize",
        dest="capitalize",
    ...
    # Analysis mode
    parser.add_argument(
        "--analyze", type=str, help="Analyze strength of existing password"
    )
    ...
    if args.json:
        ...
        for pwd in passwords:
            ...
            output.append(
                {"password": pwd, "length": len(pwd), "strength": asdict(strength)}
            )
