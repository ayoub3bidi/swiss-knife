...
class BrokenSymlinkDetector:
    ...
    def _is_circular(self, symlink_path: Path, visited: Optional[Set[Path]] = None) -> bool:
        if visited is None:
            visited = set()
    ...
    def _scan_directory(self, path: Path, current_depth: int = 0) -> None:
        ...
        for item in items:
            try:
                ...
                if item.is_dir() and not (item.is_symlink() and not self.follow_symlinks):
    ...
    def print_results(self, show_valid: bool = False, show_circular: bool = True) -> None:
        print("\n" + "=" * 70)
    ...
    def delete_broken(self, use_trash: bool = True, dry_run: bool = False) -> None:
        ...
        links_to_delete = self.broken_links + (self.circular_links if self.check_circular else [])
...
Examples:
  ...
  python broken_symlinks.py /path/to/check --delete --use-trash
    ...
    parser.add_argument('paths', nargs='+', type=Path,
                       help='Directories or symlinks to check')
    
    parser.add_argument('--follow-symlinks', action='store_true',
                       help='Follow symlinks during directory traversal')
    parser.add_argument('--check-circular', action='store_true', default=True,
                       help='Check for circular symlink references (default: True)')
    parser.add_argument('--max-depth', type=int,
                       help='Maximum recursion depth')
    
    parser.add_argument('--show-valid', action='store_true',
                       help='Display valid symlinks in output')
    parser.add_argument('--hide-circular', action='store_true',
                       help='Hide circular symlinks from output')
    
    parser.add_argument('--delete', action='store_true',
                       help='Delete broken symlinks')
    parser.add_argument('--use-trash', action='store_true', default=True,
    ...
    parser.add_argument('--file', type=str, default='broken_symlinks_report',
    ...
    try:
        ...
        detector.print_results(
            ...
            show_circular=not args.hide_circular
        )
