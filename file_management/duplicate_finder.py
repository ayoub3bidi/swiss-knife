...
class DuplicateFinder:
    def __init__(
        ...
        self.extensions = (
            {ext.lower().lstrip(".") for ext in extensions} if extensions else None
        )
    ...
    def find_duplicates(
        self, search_paths: List[Path], recursive: bool = True
    ) -> Dict[str, List[Path]]:
        """
        Args:
        ...
        """
        print(
            f"Scanning for duplicates using {self.hash_algorithm.upper()} algorithm..."
        )
...
Examples:
  ...
  %(prog)s /path/to/search --delete-duplicates --keep-strategy shortest_name
    ...
    parser.add_argument(
        "--min-size", type=str, default="0", help="Minimum file size (e.g., 1MB, 500KB)"
    )
    parser.add_argument("--max-size", type=str, help="Maximum file size (e.g., 100MB)")
    parser.add_argument(
        "--extensions", type=str, help="Comma-separated file extensions to include"
    )
    parser.add_argument(
        "--recursive",
        action="store_true",
    ...
    parser.add_argument(
        "--file", type=str, help="Output file path (prints to stdout if not specified)"
    )
    parser.add_argument(
        "--delete-duplicates",
        action="store_true",
            ...
            if args.delete_duplicates:
                if args.dry_run:
                    ...
                    for files in duplicates.values():
                        if len(files) > 1:
                            to_keep = (
                                files[0] if args.keep_strategy == "first" else files[-1]
                            )
                            for file_path in files:
                                if file_path != to_keep:
                                    print(f"  {file_path}")
