...
class BulkRenamer:
    ...
    def export_results(self, output_file: str, format: str = "json") -> None:
        data = {
            ...
            "stats": self.stats,
            "renames": [
                {"old": str(old), "new": str(new)} for old, new in self.rename_map
            ],
            "errors": [
                {"file": str(path), "error": error} for path, error in self.errors
            ],
        }
...
Special placeholders in replacement:
  ...
  \\1, \\2  - Regex capture groups
    ...
    parser.add_argument(
        "replacement",
        help="Replacement string (supports regex groups and placeholders)",
    )
    ...
    parser.add_argument(
        "--preview", action="store_true", help="Preview changes without renaming"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "-I", "--interactive", action="store_true", help="Confirm each rename operation"
    )
