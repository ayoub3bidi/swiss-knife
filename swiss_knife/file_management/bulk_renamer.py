...
from ..core import validate_path, confirm_destructive_action, safe_filename, SafetyError
...
class BulkRenamer:
    ...
    def add_pattern(self, pattern: str, replacement: str, 
                   target_dir: Path, recursive: bool = False) -> List[Tuple[Path, Path]]:
        ...
        if existing:
            raise SafetyError(f"Target files already exist: {[str(p) for p in existing]}")
...
def bulk_rename(pattern: str, replacement: str, target_dir: str,
